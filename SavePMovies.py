...
def save_species_to_csv(inp_path, species, out_path, E_cutoff_MeV=1, decimation_ratio=0.1):
    ...
    try:
        id = all_data.__getattribute__(
            f"Particles_ID_subset_{species.value}PMovie_{species.value}"
        ).data
    except AttributeError:
        id = None

    # These are always written
    weights = all_data.__getattribute__(
        f"Particles_Weight_subset_{species.value}PMovie_{species.value}"
    ).data
    x, y, z = all_data.__getattribute__(
        f"Grid_Particles_subset_{species.value}PMovie_{species.value}"
    ).data
    vx = all_data.__getattribute__(
        f"Particles_Vx_subset_{species.value}PMovie_{species.value}"
    ).data
    vy = all_data.__getattribute__(
        f"Particles_Vy_subset_{species.value}PMovie_{species.value}"
    ).data
    vz = all_data.__getattribute__(
        f"Particles_Vz_subset_{species.value}PMovie_{species.value}"
    ).data
...
def combine_frames_to_leaving_particles(inp_folder, x_cutoff, out_path=None):
    ...
    data = pd.DataFrame(
        columns=["id", "weights", "x", "y", "z", "vx", "vy", "vz", "energy", "leaving_frame"]
    )
