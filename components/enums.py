...
class Quantity(Enum):
    ...
    def get_prefix(self, data_dir_path: str):
        ...
        if self in (Quantity.Ex, Quantity.Ey, Quantity.Ez, Quantity.Bx, Quantity.By, Quantity.Bz):
            tentative_prefix = "fmovie"
        elif self in (Quantity.TEMPERATURE, Quantity.NUMBER_DENSITY, Quantity.CHARGE_DENSITY, Quantity.AVG_PARTICLE_ENERGY, Quantity.Px):
        ...
        raise ValueError(f"Flie prefix not found for quantity {self.value} in {data_dir_path}")
