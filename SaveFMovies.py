...
def save_vector_frames(inp_folder: str, frame_slice: slice, vector_field: Vector=Vector.Ey, plane: Plane = Plane.XY, out_folder: str = None):
    prefix = 'fmovie'
...
if __name__ == '__main__':
    # input paths
    input_dir = r'/fs/scratch/PAS2137/wang15032/EpochOutput/20220511_3D_Exp800nm8CB_1e21_22deg'
    # output paths
    output_dir = 'data/3d/'
    # save vector movies
    for vector in [Vector.Ex, Vector.Ey, Vector.Ez, Vector.Bx, Vector.By, Vector.Bz]:
        for plane in [Plane.XY, Plane.XZ, Plane.YZ]:
            save_vector_frames(input_dir, slice(None, None, 10), vector, plane, output_dir)