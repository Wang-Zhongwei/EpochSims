...
def save_scalar_frames(inp_folder: str, frame_slice: slice, scalar_field: Scalar=Scalar.NUMBER_DENSITY, species: Species = Species.PROTON, plane: Plane = Plane.XY, out_folder: str = None, save_grid: bool = False):
    ...
    out_file_name = f'{species.value}_{scalar_field.value}_{prefix}_{plane.value}_{start}_{stop}_{step}.npy'

    # Load grid 
    grid = sh.getdata(os.path.join(inp_folder, f'{prefix}_{frames[0]:04d}.sdf'), verbose=False).Grid_Grid_mid.data
...
if __name__ == '__main__':
    ...
    for species in Species:
        save_scalar_frames(inp_folder, slice(None),  scalar_field=Scalar.TEMPERATURE, species=species, out_folder=out_folder, plane=Plane.YZ)
        save_scalar_frames(inp_folder, slice(None),  scalar_field=Scalar.NUMBER_DENSITY, species=species, out_folder=out_folder, plane=Plane.YZ)
